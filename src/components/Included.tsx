...
export default function Included() {
  return (
    <section id="included" className="py-16 md:py-32 bg-mist-black px-4 sm:px-8 md:px-24">
      <div className="flex items-center gap-4 sm:gap-8 mb-12 md:mb-24">
        <h2 className="text-4xl sm:text-6xl md:text-8xl font-display tracking-tight opacity-80">WHAT'S INCLUDED</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {includedItems.map((item, idx) => (
          <motion.div
            ...
            className="glass-panel p-6 sm:p-10 group relative overflow-hidden h-[260px] sm:h-[300px] flex flex-col justify-between"
          >
            {/* Glow Effect */}
            ...
            <div className="space-y-3 sm:space-y-4">
              <h3 className="text-xs sm:text-sm tracking-[0.3em] font-medium uppercase text-kimono-white/60 group-hover:text-lime-accent transition-colors">
                {item.title}
              </h3>
              <p className="text-lg sm:text-xl font-editorial italic leading-snug">
                {item.desc}
              </p>
            </div>
