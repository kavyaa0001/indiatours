  ...
  return (
    <Link key={pkg.id} to={seatData?.isSoldOut ? "#" : `/package/${pkg.id}`}>
      <motion.div
        ...
        className={`group relative h-[450px] bg-white/5 border border-white/10 overflow-hidden rounded-sm ${seatData?.isSoldOut ? "cursor-not-allowed" : "cursor-pointer"}`}
      >
    ...
    <main className="min-h-screen bg-mist-black text-kimono-white selection:bg-lime-accent selection:text-mist-black">
      ...
      <section className="pt-40 pb-24 px-8 md:px-24 max-w-7xl mx-auto">
        <motion.div
          ...
          className="mb-24"
        >
          <h1 className="text-6xl md:text-8xl font-display tracking-tight mb-6">ALL PACKAGES</h1>
          <p className="text-xl text-mouse-gray font-light max-w-2xl">
      ...
      {/* Upcoming Packages */}
      <section className="py-24 px-8 md:px-24 bg-black/50 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-8 mb-16">
            <h2 className="text-4xl font-editorial italic text-white/50">Coming Soon</h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {upcomingPackages.map((pkg, idx) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: idx * 0.1 }}
                className="relative h-[300px] border border-white/5 overflow-hidden rounded-sm group grayscale"
              >
                <div className="absolute inset-0">
                  <img
                    src={pkg.image}
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <h3 className="text-3xl font-display text-white/40 mb-2">{pkg.title}</h3>
